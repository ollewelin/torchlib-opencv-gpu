...
//-------------------------------------
    ...
    t5 = t5.reshape({3,2});
    std::cout << t5 << std::endl;
    torch::Tensor a1_cpu = torch::tensor(5.6, torch::kFloat32);
    ...
    std::cout << "Make CPU tensor gradient calculation " << '\n';
    c1_cpu.backward();// Compute the gradients
    std::cout << "a1_cpu gradient = " << a1_cpu.grad() << '\n';
    std::cout << "NOTE! b1_cpu gradient is disabled " << '\n';
    std::cout << "b1_cpu gradient = " << b1_cpu.grad() << '\n';
    
    std::cout << "PyTorch Basics\n\n";
    std::cout << std::fixed << std::setprecision(4);

    torch::Tensor a1_gpu = torch::tensor(5.6, torch::kFloat32).to(device);
    torch::Tensor b1_gpu = torch::tensor(2.8532, torch::kFloat32).to(device);
    torch::Tensor c1_gpu = torch::tensor(0.0, torch::kFloat32).to(device);
    a1_gpu.set_requires_grad(true);
    b1_gpu.set_requires_grad(true);
    c1_gpu.set_requires_grad(true);
    std::cout << "Make GPU tensor forward calculation " << '\n';
    c1_gpu = a1_gpu * b1_gpu;

    std::cout << "c1_gpu = a1_gpu * b1_gpu = " << c1_gpu << '\n';
    std::cout << "Make GPU tensor gradient calculation " << '\n';
    c1_gpu.backward({}, true);// Compute the gradients
    std::cout << "a1_gpu gradient = " << a1_gpu.grad() << '\n';
    std::cout << "b1_gpu gradient = " << b1_gpu.grad() << '\n';
